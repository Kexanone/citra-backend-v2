...
class LobbyConfig(AppConfig):
    ...
    def ready(self):
        ...
        self.scheduler.add_job(
            delete_timed_out_rooms,
            'interval',
            seconds=settings.MAX_ROOM_UPDATE_TIMEOUT_S,
        )
...
def delete_timed_out_rooms():
    ...
    current_time = time()
    timed_out_rooms = Room.objects.filter(
        timestamp__lt=current_time - settings.MAX_ROOM_UPDATE_TIMEOUT_S
    )
