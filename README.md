...
It is recommended to put the app behind a reverse proxy, such as nginx, configured with HTTPS. IP of the client should be forwarded via `X-Real-IP` and the protocol via `X-Forwarded-Proto` proxy headers.
...
Pass backend URL, created account user name and token as arguments:
```sh
